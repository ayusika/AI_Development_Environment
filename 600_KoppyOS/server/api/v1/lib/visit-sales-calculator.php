...
/*
 * Kohaku Work
 * Visit sales shared calculator / confirmer
 *
 * このファイル自身では
 * トランザクションを開始・commitしない。
 *
 * 日次一括確定側で
 * 同じPDO上の複数確定を
 * 1 transaction にまとめられるようにする。
 */


function koppyFetchVisitSalesCourseRate(
    PDO $pdo,
    int $storeCourseId,
...
function koppyFetchVisitSalesOptionRate(
    PDO $pdo,
    int $storeId,
    int $optionId,
    string $effectiveDate
...
function koppyReadVisitSalesIntegerOverride(
    array $payload,
    string $key,
    int $fallback
): int {
...
function koppyCalculateVisitSales(
    PDO $pdo,
    int $visitId,
    array $overrides = []
): array {
     ...
     * 渡された場合だけ入力値を反映する。
     *
     * 日次一括確定でoverridesを渡さなければ、
     * 現在DBに保存済みのdraft値をそのまま使う。
     */
    if ($overrides !== []) {
    ...
    if ($storeCourseId > 0) {
        ...
        $courseRate =
            koppyFetchVisitSalesCourseRate(
                $pdo,
                $storeCourseId,
                $effectiveDate
            );
    ...
    foreach (
        ...
        if ($optionId > 0) {
            ...
            $optionRate =
                koppyFetchVisitSalesOptionRate(
                    $pdo,
                    $storeId,
                    $optionId,
                    $effectiveDate
...
function koppyConfirmVisitSales(
    PDO $pdo,
    int $visitId,
    array $overrides = []
): array {
