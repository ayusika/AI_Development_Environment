...
KoppyOSでは、
API Key、Client Secret、Access Token、Refresh Token、Passwordなど、
第三者へ公開すべきでない認証情報をSecret情報として扱う。

### 基本原則

1. Secret情報の実値をChatGPT、LINE、Discord等の会話欄へ貼らない。

2. Secret情報の実値をGitHubへ保存しない。
   Public / Private Repositoryを問わず、
   設計書やソースコードへ直接記述しない。

3. Koppyとの会話ではSecret情報の実値ではなく、
   `GITHUB_TOKEN`
   `OPENAI_API_KEY`
   などの識別名を使用する。

4. Secret情報が必要な場合は、
   Server側Private領域、環境変数、GitHub Secretsなど、
   実行環境側の安全な保存領域から読み込む。

5. Secret情報を誤って会話・GitHub・その他公開または共有され得る場所へ貼った場合、
   単純な削除だけで安全になったとは判断しない。
   漏洩した可能性があるSecretは原則として失効・再発行（Rotate）する。

6. Koppyは、
   Secret情報の実値を会話欄へ貼るようしいちゃんへ要求しない。
   Secretが必要な作業では、安全な保存場所へ設定する手順を優先して案内する。

7. Secret情報をソースコードへ直接ハードコードしない。
   原則として環境変数またはPrivate設定から参照する。
...
例：
    ...
    'github_token' => getenv(♦︎♦︎'GITHUB_TOKEN'♦︎♦︎)
...
`♦︎♦︎` はKoppyからしいちゃんへの
「ここを編集する」という説明用マーカーである。

実際の完成コード・設定値・ファイル名には含めない。
...
この表示ルールはSecret情報だけに限定しない。

Koppyがしいちゃんへ手動操作を依頼する際に、
...
など、操作対象を明確にする必要がある場合に使用する。

目的は、
しいちゃんが説明全体を読み解かなくても
「どこを操作するのか」を視覚的に即座に判別できる状態を作ることである。
