...
## 10.6 Writer Presentation Format
...
このPresentation Formatは、
Writer Instruction Formatの内容要件を変更するものではなく、
ユーザーへ提示する際の
表示単位およびコピー単位を定義する。

### Code Fence Safety

Koppy OSパネルへ表示する実データに
Markdownコードフェンスが含まれる場合は、
表示用の外側コードフェンスと
実データ内のコードフェンスを衝突させてはならない。

必要に応じて、
外側のコードフェンスへ
実データ内より多いバッククォート数を使用する。
...
を、
Writerへ渡す実データへ混入させてはならない。

コピー操作によって取得される内容が、
そのWriter項目の実データだけになるようにする。
